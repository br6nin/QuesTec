'use client' // IMPORTANTE: Adiciona a diretiva para usar hooks (como useRouter)
...
import { PenTool, ArrowLeft, CheckCircle, AlertCircle, TrendingUp, Download, Share2, CornerRightUp } from 'lucide-react'

// Interface de dados esperada do Back-end
...
const EssayFeedbackPage = ({ params }: { params: { id: string } }) => {
      ...
      if (response.ok) {
        ...
        if (data.essay.feedback) {
          ...
          try {
            ...
            correctionData = { 
                finalScore: data.essay.finalScore || 0,
                generalFeedback: "Erro na leitura da correção. Tente corrigir novamente.", 
                suggestions: [],
                c1Score: data.essay.c1Score || 0,
                c2Score: data.essay.c2Score || 0,
                c3Score: data.essay.c3Score || 0,
                c4Score: data.essay.c4Score || 0,
                c5Score: data.essay.c5Score || 0,
            };
          }
  ...
  const getProgressColor = (score: number) => {
    // Retorna a classe completa para que o Tailwind possa a reconhecer
  ...
  // Função para aplicar a cor do indicador (usada no componente Progress)
  const getProgressIndicatorClassName = (score: number, maxScore: number = 1000) => {
    const percentage = (score / maxScore) * 100;
    const colorClass = getProgressColor(score * (1000/maxScore)); // Ajusta a pontuação para a escala de 1000 para a cor
    
    // IMPORTANTE: Adiciona a classe de cor ao Progress.Indicator. 
    // Isso requer uma pequena modificação no progress.tsx (veja abaixo)
    return `h-full w-full flex-1 transition-all ${colorClass}`;
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8 md:py-12">
        <div className="max-w-7xl mx-auto">
          ...
          {activeTab === 'feedback' && correction && (
            // Classe GRID principal para 3 colunas em telas grandes
                ...
                <Card className="shadow-2xl border-t-4 border-red-500 transform transition duration-300 hover:scale-[1.01] mb-6">
                  <CardContent className="p-8 text-center">
                    ...
                    <Progress 
                      ...
                      // Aplica a classe de cor ao elemento raiz do Progress
                      className={`mt-6 h-3 [&>*]:${getProgressColor(correction.finalScore)}`} 
                      // Nota: A classe [&>*] pode ser necessária dependendo da sua versão do Progress.tsx. 
                      // Se não funcionar, confira a Solução 3.
                    />
                  </CardContent>
                </Card>
                  ...
                  <Card className="shadow-md">
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between text-lg">
                        ...
                        <span className={`text-2xl font-black ${getScoreColor(correction.c1Score * 5)}`}>
                          {correction.c1Score}
                        </span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <Progress 
                        value={(correction.c1Score / 200) * 100} 
                        className={`mb-3 [&>*]:${getProgressColor(correction.c1Score * 5)}`} 
                      />
                  ...
                  <Card className="shadow-md">
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between text-lg">
                        ...
                        <span className={`text-2xl font-black ${getScoreColor(correction.c2Score * 5)}`}>
                          {correction.c2Score}
                        </span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <Progress 
                        value={(correction.c2Score / 200) * 100} 
                        className={`mb-3 [&>*]:${getProgressColor(correction.c2Score * 5)}`} 
                      />
                  ...
                  <Card className="shadow-md">
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between text-lg">
                        ...
                        <span className={`text-2xl font-black ${getScoreColor(correction.c3Score * 5)}`}>
                          {correction.c3Score}
                        </span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <Progress 
                        value={(correction.c3Score / 200) * 100} 
                        className={`mb-3 [&>*]:${getProgressColor(correction.c3Score * 5)}`} 
                      />
                  ...
                  <Card className="shadow-md">
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between text-lg">
                        ...
                        <span className={`text-2xl font-black ${getScoreColor(correction.c4Score * 5)}`}>
                          {correction.c4Score}
                        </span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <Progress 
                        value={(correction.c4Score / 200) * 100} 
                        className={`mb-3 [&>*]:${getProgressColor(correction.c4Score * 5)}`} 
                      />
                  ...
                  <Card className="shadow-md">
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between text-lg">
                        ...
                        <span className={`text-2xl font-black ${getScoreColor(correction.c5Score * 5)}`}>
                          {correction.c5Score}
                        </span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <Progress 
                        value={(correction.c5Score / 200) * 100} 
                        className={`mb-3 [&>*]:${getProgressColor(correction.c5Score * 5)}`} 
                      />
